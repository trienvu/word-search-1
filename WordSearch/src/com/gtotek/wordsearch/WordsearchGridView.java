...
public class WordsearchGridView extends LinearLayout implements OnKeyListener {
	...
	public void clearSelection() {
		if (mSelectionDirection != null && mSelectionSteps != null) {
			for (View view : getSelectionViews()) {
				((TextView) view.findViewById(R.id.lbl_char))
						.setTextColor(mDefaultColor);
			}
			mOnWordSelectedListener.onWordSelected(getPositionsOnPath(
					mSelectionDirection, mSelStartPosition, mSelectionSteps));
		}
	...
	public int getEndPosition(Direction direction, int startPosition, int steps) {
		...
		for (int i = steps; i >= 0; i--) {
			...
			if (curRow < 0 || curCol < 0 || curRow >= mRows
					|| curCol >= mColumns) {
				break;
			}
		}
	...
	public List<Integer> getPositionsOnPath(Direction direction,
			int startPosition, int steps) {
	...
	public boolean onKey(View v, int keyCode, KeyEvent event) {
		if (event.getAction() == KeyEvent.ACTION_UP
			...
			TextView tv = (TextView) ((ViewGroup) getFocusedChild())
					.getFocusedChild().findViewById(R.id.lbl_char);
			if (tv != null) {
	...
	public void onRestoreInstanceState(Parcelable state) {
		if (state instanceof WordsearchGameState) {
			super.onRestoreInstanceState(((WordsearchGameState) state)
					.getSuperState());
	...
	public boolean selectionChanged(float xPos, float yPos) {
		if (mSelStartPosition == null) {
			...
			mSelectionDirection = Direction.getDirection((float) Math.atan2(
					yDelta, xDelta));

			float stepSize = mSelectionDirection.isAngle() ? (float) Math
					.hypot(mColumnWidth, mColumnWidth) : mColumnWidth;
			...
			if (mSelectionDirection != previousDirection
					|| mSelectionSteps != previousSteps) {
	...
	public void setBoard(char[][] board) {
		...
		for (int i = 0; i < board.length; i++) {
			for (int j = 0; j < board[i].length; j++) {
				...
				((TextView) v.findViewById(R.id.lbl_char)).setText(""
						+ board[i][j]);
			}
		}
	}
	 ...
	 */
	public void setOnWordSelectedListener(
	...
	public void wordFound(Word word) {
		if (!mGameState.mFoundWords.contains(word)) {
			...
			if (mFoundCache == null) {
				mFoundCache = Bitmap.createBitmap(getMeasuredWidth(),
						getMeasuredHeight(), Bitmap.Config.ARGB_8888);
			}
			Canvas foundCanvas = new Canvas(mFoundCache);
			Random rnd = new Random();
			mFoundPaint.setARGB(155, rnd.nextInt(256), rnd.nextInt(256), rnd.nextInt(256));
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mSelectionDirection != null && mSelectionSteps != null
		...
		} else if (!isInTouchMode() && mSelStartPosition != null
				&& mFocusSelected) {
			...
			canvas.drawRoundRect(superRect, mCornerRadius, mCornerRadius,
					mPaint);
			canvas.restore();
		}
...
//	}
	...
	private List<View> getSelectionViews() {
		...
		for (Integer position : getPositionsOnPath(mSelectionDirection,
				mSelStartPosition, mSelectionSteps)) {
	...
	private void init() {
		...
		mSelectionColor =   Color.parseColor("#FF0000") ;
		...
		for (int i = 0; i < mColumns; i++) {
			...
			for (int j = 0; j < mColumns; j++) {
				View view = LayoutInflater.from(getContext()).inflate(
						R.layout.wordsearch_grid_cell, null);
				...
				view.setOnFocusChangeListener(new OnFocusChangeListener() {
					public void onFocusChange(View v, boolean hasFocus) {
						...
						((TextView) v.findViewById(R.id.lbl_char))
								.setTextColor(hasFocus ? 0xFF00870a
										: mDefaultColor);
					}
				});
	...
	private static final class WordsearchGameState extends BaseSavedState {
		...
		private WordsearchGameState(Parcel in) {
			super(in);
			mFoundWords = new HashSet<Word>();
			Parcelable[] parcels = in.readParcelableArray(Word.class
					.getClassLoader());
